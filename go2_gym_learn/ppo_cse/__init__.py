...
class Runner:
    ...
    def learn(self, num_learning_iterations, init_at_random_ep_len=False, eval_freq=100, curriculum_dump_freq=500, eval_expert=False):
        ...
        cur_reward_sum = torch.zeros(self.env.num_envs, dtype=torch.float, device=self.device) #当前奖励总和初始化
        cur_episode_length = torch.zeros(self.env.num_envs, dtype=torch.float, device=self.device) #当前回合总和初始化

        tot_iter = self.current_learning_iteration + num_learning_iterations #总迭代
        for it in range(self.current_learning_iteration, tot_iter):
            ...
            with torch.inference_mode():
                for i in range(self.num_steps_per_env):
                    ...
                    if 'curriculum' in infos:
                        ...
                        cur_reward_sum += rewards #奖励叠加
                        cur_episode_length += 1 #当前回合长度自增1
                ...
                # Learning step
                start = stop
                self.alg.compute_returns(obs_history[:num_train_envs], privileged_obs[:num_train_envs]) #根据历史信息计算返回值
            ...
            if it % RunnerArgs.save_interval == 0:
                with logger.Sync():
                    logger.torch_save(self.alg.actor_critic.state_dict(), f"checkpoints/ac_weights_{it:06d}.pt") #训练好的权重保存一次
